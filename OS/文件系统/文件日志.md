...
normal块由一个descriptor块 + 多个数据/元数据块组成。用于存储文件系统数据或元数据的日志块。它们记录了对文件系统状态的实际修改。在事务过程中，所有对文件系统进行的写操作都会先记录在normal块中。这些块在成功提交后会更新到实际的文件系统。在一个文件被写入时，写入的数据会首先存储在正常块中，确保在事务未提交时，数据的完整性和一致性。
...
T_LOCKED（第351行）：当事务开始提交时，事务的状态被设置为T_LOCKED。此状态表示事务已经锁定，不能再向该事务中添加新的原子操作。这确保了当前事务不再接受新的修改操作。

T_FLUSH（第430行）：事务进入T_FLUSH状态时，表示事务的写入操作开始，事务中的数据块缓冲区将被写回磁盘。这是事务提交的初始阶段。

T_COMMIT（第524行）：事务进入T_COMMIT状态时，表示事务的元数据缓冲区正在被写入日志。这阶段主要负责写入事务中的元数据以确保文件系统的一致性。

T_COMMIT_RECORD（第792行）：事务进入T_COMMIT_RECORD状态时，表示事务即将完成，提交块（commit block）已经被写入日志中。这是提交的最后阶段，确保日志中记录了提交成功的状态。

T_FINISHED（第934行）：事务进入T_FINISHED状态时，表示事务已经完全提交，所有数据和元数据都已经写入日志，并且事务已经完成。

Checkpoint处理（第930行后）：提交完成后，事务将被插入到j_checkpoint_transactions队列中，并等待检查点操作。在这种状态下，事务要么等待被清理出内存（如果checkpoint为空），要么保持在检查点队列中以便后续处理。
